...
class EvalBias(BaseEvalType):
    ...
    def evaluate(
        ...
        **kwargs,
        ...
        #         # )
        #
        #     checkpoint_path = Path(saved_model_path) / "checkpoints" / "model_best.pth"
        #     if checkpoint_path.exists():
        #         # Restore the model and necessary variables from checkpoint
        #         checkpoint = torch.load(checkpoint_path)
        #         model.load_state_dict(checkpoint['model_state_dict'])
        #         # If other variables like optimizer or epoch are needed, load them similarly
        #         epoch = checkpoint['epoch']
        #         optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        #         criterion.load_state_dict(checkpoint['loss_state_dict'])
        #
        #         #TODO: rether save the eval stores directly in the checkpointed model
        #         ## Return the evaluation scores from the checkpointed model
        #         #eval_scores = checkpoint.get('eval_scores', {})
        #
        #         # Now, recompute the evaluation scores from the validation set
        #         eval_scores = cls.compute_eval_metrics(model, dataset, saved_model_path,
